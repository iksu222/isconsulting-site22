    ...
    <div className="min-h-screen bg-white text-gray-900">
      <nav className="sticky top-0 bg-white shadow z-10 p-4 flex justify-between">
        ...
        <div className="space-x-4">
          ...
          <button onClick={() => setPage("about")}>회사소개</button>
          <button onClick={() => setPage("review")}>상담후기</button>
          <button onClick={() => setPage("apply")}>상담신청</button>
          <button onClick={() => setPage("contact")}>연락처</button>
        </div>
      </nav>
      ...
      {page === "apply" && (
        <div className="p-8 space-y-4">
          ...
          <form className="space-y-4 max-w-md mx-auto">
            ...
            <button type="submit" className="bg-black text-white px-4 py-2 rounded">빠른 상담 요청하기</button>
          </form>
        </div>
      )}
      ...
      {page === "contact" && (
        <div className="p-8 space-y-4">
          ...
          <p>📍 서울시 어디구 어디동</p>
          <button className="bg-yellow-400 px-4 py-2 rounded">카카오톡 상담하기</button>
        </div>
      )}
    </div>
  );
